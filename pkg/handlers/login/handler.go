...
func NewHandler(service auth.Service, logger zerolog.Logger) *Handler {
	l := logger.With().Str("component", "loginHandler").Logger()
...
func (h *Handler) Login(w http.ResponseWriter, r *http.Request) {
	...
	tokens, err := h.service.Login(user.Email, user.Password)
	if err != nil {
		h.logger.Error().Msgf("failed to login user %v: %v", user.Email, err)
		switch err {
		case auth.ErrNotExist:
			h.respond(w, errorMessage{Error: auth.ErrNotExist.Error()}, http.StatusForbidden)
			return
		case auth.ErrWrongPassword:
			h.respond(w, errorMessage{Error: auth.ErrWrongPassword.Error()}, http.StatusForbidden)
			return
		default:
	...
	resp := response{Message: "user successfully login", AccessToken: tokens.AccessToken,
